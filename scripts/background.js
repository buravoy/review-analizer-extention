...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.from === 'popup') {
        switch (msg.subject) {
            case 'get_data':
                return sendResponse(dataSet[msg.tabId]);
        }
        return;
    }
    ...
    if (msg.from === 'content') {
        switch (msg.subject) {
            case 'init':
                return dataSet[sender.tab.id] = {};
            case 'enable':
                return enablePopup(sender.tab.id);
            case 'disable':
                return disablePopup();
            case 'dataset':
                return prepareDataset(sender.tab.id, msg)
        }
    }
});